...
function showDiff(repoCache: string, from: string, to: string, subPath: string | null, isFile: boolean): void {
  try {
    ...
    if (isFile && subPath) {
      ...
      if (diff) {
        ...
        console.log(color.dim(lines.map((l) => `    ${l}`).join("\n")));
        if (diff.split("\n").length > 20) console.log(color.dim("    ... (truncated)"));
      }
    } else {
      const stat = execSync(
      ...
      if (stat) console.log(color.dim(stat.split("\n").map((l) => `    ${l}`).join("\n")));
    }
  } catch { /* no diff */ }
}
...
export async function diffCommand(name?: string): Promise<void> {
  ...
  for (const [n, specifier] of skills) {
    ...
    if (currentCommit && latestCommit === currentCommit) {
      console.log(`  ${color.dim(`${n} up to date (${latestCommit.slice(0, 7)})`)}`);
      continue;
    }
    ...
    if (currentCommit) showDiff(repoCache, currentCommit, latestCommit, baseResolved.subPath, false);
  }

  for (const [n, entry] of docs) {
    const resolved = parseSource(entry.source);
    const repoCache = getRepoCache(resolved.repoUrl);
    const baseResolved = parseSource(entry.source.replace(/@[0-9a-f]{7,}$/, ""));
    ...
    const currentCommit = resolved.ref.match(/^[0-9a-f]{7,}$/) ? resolved.ref : null;
    if (currentCommit && latestCommit === currentCommit) {
      console.log(`  ${color.dim(`doc:${n} up to date (${latestCommit.slice(0, 7)})`)}`);
      continue;
    }
    ...
    if (currentCommit) showDiff(repoCache, currentCommit, latestCommit, baseResolved.subPath, true);
  }
