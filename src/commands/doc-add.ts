...
export function docAddCommand(specifier: string, opts: DocAddOptions): void {
  ...
  if (opts.targets.length === 0) {
    log.warn("No targets specified. Use --targets to specify where this doc applies.");
    log.dim(`  Example: vulyk doc-add nicobailon/visual-explainer/docs/usage.md --targets ".claude/skills"`);
    process.exit(1);
  }

  const name = specifier.split("/").filter(Boolean).pop()?.replace(/\.md$/, "").replace(/@.*$/, "") ?? specifier;
