...
export function fetchSource(resolved: ResolvedSource, destDir: string): string {
  ...
  const repoHash = Buffer.from(resolved.repoUrl)
    .toString("base64url")
    .slice(0, 32);
  ...
  const archivePath = path.join(os.tmpdir(), `vulyk-${String(Date.now())}.tar`);
  execSync(
    `git --git-dir="${repoCache}" archive --format=tar -o "${archivePath}" "${archiveTarget}"`,
    { stdio: "pipe" },
  );
