...
export function updateCommand(name) {
    ...
    let updated = 0;
    let upToDate = 0;
    for (const [n, specifier] of toUpdate) {
        const resolved = parseSource(specifier);
        const repoCache = getRepoCache(resolved.repoUrl);
        // Strip pinned commit to resolve the branch/tag ref
        ...
        let latestCommit;
        try {
            latestCommit = execSync(`git --git-dir="${repoCache}" rev-parse "${baseResolved.ref}"`, { encoding: "utf8", stdio: "pipe" }).trim();
        }
        catch {
        ...
        try {
            ...
            install(n, tmpDir, manifest.paths);
            fs.rmSync(tmpDir, { recursive: true, force: true });
            // Pin to new commit in vulyk.json
    ...
    console.log("");
    if (updated > 0)
        log.success(`Updated ${String(updated)} skill(s)`);
    if (upToDate > 0)
        log.dim(`${String(upToDate)} already up to date`);
}
