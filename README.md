...
The match is against each entry's `targets`. The same targets determine where Vulyk generates agent files and which files the document applies to.
...
| `entries.<name>.targets`     | Optional paths or globs that determine both where agent files are generated and which files match the entry in `find-docs`/`find-targets` |
